<?php
/*
Plugin Name: VarkTech Maximum Purchase for WP E-Commerce
Plugin URI: http://varktech.com
Description: An e-commerce add-on for WP-E-Commerce, supplying maximum purchase functionality.
Version: 1.06                            
Author: Vark
...
class VTMAX_Controller{
    ...
    require ( VTMAX_DIRNAME . '/wpsc-integration/vtmax-parent-definitions.php');
 ...
 *************************************************** */
    ...
    require ( VTMAX_DIRNAME . '/wpsc-integration/vtmax-parent-functions.php');
    require ( VTMAX_DIRNAME . '/wpsc-integration/vtmax-parent-cart-validation.php');
  ...
  function vtmax_activation_hook() {
   ...
   if (version_compare(PHP_VERSION, VTMAX_EARLIEST_ALLOWED_PHP_VERSION) < 0) {    //'<0' = 1st value is lower 
    ...
    if(defined('WOOCOMMERCE_VERSION') && (VTMAX_PARENT_PLUGIN_NAME == 'WooCommerce')) { 
      ...
      if( (version_compare(strval($new_version), strval($current_version), '>') == 1) ) {   //'==1' = 2nd value is lower 
  			// delete_option('vtmax_setup_options');
  			 wp_die( __('<strong>Looks like you\'re running an older version of WooCommerce. <br>You need to be running at least ** WooCommerce 3.8 **, to use the Varktech Maximum Purchase plugin.</strong>', 'vtmax'), __('VT Maximum Purchase not compatible - WooCommerce', 'vtmax'), array('back_link' => true));
  			return;
  		}
    }   else 
